<?php

declare(strict_types=1);

require_once DOL_DOCUMENT_ROOT . '/core/modules/DolibarrModules.class.php';
require_once DOL_DOCUMENT_ROOT . '/custom/immocore/core/modules/modimmocore.class.php';

class modImmobien extends DolibarrModules
{
    public function __construct($db)
    {
        global $langs, $conf;

        $this->db = $db;
        $this->numero = 700001;
        $this->rights_class = 'immobien';
        $this->family = "other";
        $this->module_position = '90';
        $this->name = preg_replace('/^mod/i', '', get_class($this));
        $this->description = "Gestion des biens immobiliers";
        $this->version = '1.0.0';
        $this->const_name = 'MAIN_MODULE_' . strtoupper($this->name);
        $this->picto = 'building';
        $this->module_parts = array();
        $this->dirs = array();
        $this->config_page_url = array("immobien@immobilier");
        $this->depends = array('mod_immocore' => 1, 'mod_societe' => 1);
        $this->requiredby = array();
        $this->conflictwith = array();
        $this->langfiles = array("immobien@immobilier");
        $this->phpmin = array(8, 1);
        $this->need_dolibarr_version = array(23, 0);
        $this->warnings_activation = array();
        $this->warnings_activation_ext = array();

        $this->const = array();
        $this->tabs = array();
        $this->dictionaries = array();

        $this->menu = array();
        $r = 0;
        $this->menu[$r] = array(
            'fk_menu' => 'fk_mainmenu=immobilier',
            'type' => 'top',
            'titre' => 'Immobilier - Biens',
            'mainmenu' => 'immobilier',
            'leftmenu' => 'immobien',
            'url' => '/custom/immobien/index.php',
            'langs' => 'immobien@immobilier',
            'position' => 700001,
            'perms' => '1',
            'target' => '',
            'user' => 2,
        );
        $r++;

        $this->rights = array();
        $this->rights_class = 'immobien';
        $r = 0;
        $this->rights[$r][0] = 700001001;
        $this->rights[$r][1] = 'Lire les Immobilier - Biens';
        $this->rights[$r][3] = 0;
        $this->rights[$r][4] = 'read';
        $r++;
        $this->rights[$r][0] = 700001002;
        $this->rights[$r][1] = 'Créer/Modifier les Immobilier - Biens';
        $this->rights[$r][3] = 0;
        $this->rights[$r][4] = 'write';
        $r++;
        $this->rights[$r][0] = 700001003;
        $this->rights[$r][1] = 'Supprimer les Immobilier - Biens';
        $this->rights[$r][3] = 0;
        $this->rights[$r][4] = 'delete';
    }

    public function init($options = ''): int
    {
        $sql = array();
        $result = $this->loadTables();

        if ($result < 0) {
            return -1;
        }

        return $this->init($options);
    }

    public function remove($options = ''): int
    {
        $sql = array();
        return $this->remove($options);
    }
}
